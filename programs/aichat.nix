{
  ...
  aichat = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      # assist_API_KEY="$(cat /run/agenix/aiservice-api-key)";
      # aiservice_API_KEY="$(cat /run/agenix/aiservice-api-key)";
      # export OPENAI_API_KEY assist_API_KEY aiservice_API_KEY
      exec aichat "$@"
    '';
  };
in {
  ...
  '';
  home.file."${configDir}/roles/coder-assist.md".text = ''
    ---
    model: assist:assist-3-5-sonnet-latest
    ---
    Provide only code without comments or explanations. Do not wrap in markdown. Just respond with the code, nothing else.
    ### INPUT:
    async sleep in js
  ...
  '';
  home.file."${configDir}/roles/refactor-assist.md".text = ''
    ---
    model: assist:assist-3-5-sonnet-latest
    ---
    Provide only code without comments or explanations. Take the input and refactor it to be more succinct, simpler and best-practice.
  ...
  home.file."${configDir}/roles/explain-openai.md".text = ''
    ...
    of milliseconds.
  '';
  home.file."${configDir}/roles/explain-assist.md".text = ''
    ---
    model: assist:assist-3-5-sonnet-latest
    ---
    Explain what the given code does in a clear and concise manner. Use a style suitable for showing in a modern terminal.
  ...
  #     {
  #       type = "assist";
  #       api_base = "https://api.aiservice.com/v1";
  #     }
  #   ];
  # };
}
